...
initialise_linear_model_json() {
    ...
    echo "$model_config_init_json" > "$chosen_dir/model_params.json"
}

initialise_quadratic_model_json() {

    # first delete any existing model
    rm "$chosen_dir/model_params.json"
    ...
    for pair in "${pairs[@]}"; do
            ...
            jq --arg option "$pair" '.model_terms += [$option]' "$chosen_dir/model_params.json" > tmpfile && mv tmpfile "$chosen_dir/model_params.json"
    done
    ...
    for squared_var in "${squared_vars[@]}"; do
        # Read the current array from JSON, append the selected option, and update the JSON file
        jq --arg option "$squared_var" '.model_terms += [$option]' "$chosen_dir/model_params.json" > tmpfile && mv tmpfile "$chosen_dir/model_params.json"
...
delete_interaction() {
    local json_file="$chosen_dir/model_params.json"
...
add_interaction() {
    ...
    select option in "${Variables[@]}"; do
        if [ -n "$option" ]; then
            ...
            selected=$option
            # Read the current array from JSON, append the selected option, and update the JSON file
            jq --arg option "$selected" '.model_terms += [$option]' "$chosen_dir/model_params.json" > tmpfile && mv tmpfile "$chosen_dir/model_params.json"
...
screening_analysis_menu() {
    ...
    if [ -f "$chosen_dir/model_params.json" ]; then
        # spacer
        echo -e "\n"
        ...
        while true; do
            ...
            mapfile -t Terms < <(jq -r '.model_terms[]' "$chosen_dir/model_params.json")
